...
/// Result of UPI payment flow
class UpiPaymentFlowResult {
  final bool success;
  final bool cancelled;
  final String? transactionId;
  final Bill? updatedBill;

  UpiPaymentFlowResult({
    this.success = false,
...
/// UPI Payment Screen
///
/// Shows a real UPI QR code that customers can scan with any UPI app
/// (GPay, PhonePe, Paytm etc.) to pay.
class UpiPaymentScreen extends StatefulWidget {
  ...
  /// Show the UPI payment flow
  static Future<UpiPaymentFlowResult> show(
      BuildContext context, {
        required Bill bill,
      }) async {
    final result = await Navigator.push<UpiPaymentFlowResult>(
      context,
      MaterialPageRoute(
        builder: (_) => UpiPaymentScreen(bill: bill),
      ),
    );

    return result ?? UpiPaymentFlowResult(cancelled: true);
  }
...
class _UpiPaymentScreenState extends State<UpiPaymentScreen> {
  ...
  void _generateQrCode() {
    ...
    final shopName = profile.shopName ?? 'Shop';

    if (upiId == null || upiId.isEmpty) {
      setState(() {
        _errorMessage = 'UPI ID not set. Please add your UPI ID in Profile settings to accept QR payments.';
      });
      return;
    }

    _qrData = _upiService.generateUpiQrCode(
      merchantVpa: upiId,
  ...
  Future<void> _handleMarkAsPaid() async {
    ...
    try {
      ...
      if (mounted) {
        await _showSuccessDialog(txnId);
      }
    } finally {
      if (mounted) {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              Navigator.pop(
                context,
                UpiPaymentFlowResult(cancelled: true),
              );
            },
  ...
  Widget build(BuildContext context) {
    ...
    // If no UPI ID is set, show error screen
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                // Action Buttons
                const Text(
                  'After customer scans & pays, confirm payment status:',
                  style: TextStyle(
                    color: AppTheme.textSecondary,
                    fontSize: 13,
                  ),
